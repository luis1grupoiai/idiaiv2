...
# Importamos la señal de sistemas
...
class UserCoordinacionInlineForm(forms.ModelForm):
    class Meta:
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Filtros adicionales para coordinaciones si es necesario
        self.fields['coordinacion'].queryset = Coordinacion.objects.filter(estado=1)

class UserCoordinacionInline(admin.StackedInline):
    model = UserCoordinacion
    form = UserCoordinacionInlineForm
    can_delete = False
    verbose_name_plural = 'Coordinación de usuario'
    max_num = 1  # Por ser relación OneToOne

class CustomUserAdmin(UserAdmin):
    form = CustomUserChangeForm
    inlines = (UserCoordinacionInline,)
    ...
    def save_related(self, request, form, formsets, change):
        # Registra quién hizo el cambio en relaciones
        set_changed_by(request.user)
    ...
    def get_coordinacion(self, obj):
        try:
            return obj.usercoordinacion.coordinacion.nombre if obj.usercoordinacion.coordinacion else 'Sin coordinación'
        except UserCoordinacion.DoesNotExist:
            return 'Sin coordinación'
    get_coordinacion.short_description = 'Coordinación'

    def get_gerencia(self, obj):
        try:
            coordinacion = obj.usercoordinacion.coordinacion
            return coordinacion.id_gerencia.nombre if coordinacion and coordinacion.id_gerencia else 'Sin gerencia'
        except (UserCoordinacion.DoesNotExist, AttributeError):
            return 'Sin gerencia'
